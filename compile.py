...
def process_single_file(filename):
    ...
    rootData = bytes(rootData, 'utf-8')
    dnaStoreData = dnaStore.dump(verbose=args.verbose)
    dnaStoreData = str(dnaStoreData)
    dnaStoreData = bytes(dnaStoreData, 'utf-8')
    output = os.path.splitext(filename)[0] + '.pdna'
    print('Writing...', output)
    data = dnaStoreData + rootData
    if args.compress:
        import zlib
    ...
    if args.verbose:
        catalogCodeCount = 0
        for root, codes in list(dnaStore.catalogCodes.items()):
